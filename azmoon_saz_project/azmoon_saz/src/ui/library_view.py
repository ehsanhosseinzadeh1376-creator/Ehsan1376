...
from utils.pdf_exporter import export_lesson_to_pdf, export_quiz_to_pdf
...
class LibraryView(ctk.CTkFrame):
    ...
    def _make_doc_row(self, doc):
        ...
        ctk.CTkButton(btns, text=fa('📄 خروجی PDF'), width=100, height=32, corner_radius=8,
                      font=(FONT, 12), fg_color='#f2a44d', hover_color='#d68a37',
                      command=lambda d=doc: self._export_doc(d)
                      ).pack(side='right', padx=4)
    ...
    def _export_doc(self, doc):
        if not doc.get('lesson_text'):
            messagebox.showwarning('توجه', 'این سند درسنامه ندارد.')
            return
        path = filedialog.asksaveasfilename(
            defaultextension='.pdf',
            filetypes=[('PDF Files', '*.pdf')],
            initialfile=f"Lesson_{doc['title']}.pdf"
        )
        if not path:
            return
        try:
            export_lesson_to_pdf(f"درسنامه: {doc['title']}", doc['lesson_text'], path)
            messagebox.showinfo('موفق', 'درسنامه صادر شد.')
    ...
    def _make_quiz_row(self, quiz):
        ...
        ctk.CTkButton(btns, text=fa('📄 PDF'), width=70, height=32, corner_radius=8,
                      font=(FONT, 12), fg_color='#f2a44d', hover_color='#d68a37',
                      command=lambda q=quiz: self._export_quiz(q)
                      ).pack(side='right', padx=4)
    ...
    def _export_quiz(self, quiz):
        ...
        path = filedialog.asksaveasfilename(
            defaultextension='.pdf',
            filetypes=[('PDF Files', '*.pdf')],
            initialfile=f"Quiz_{quiz['title']}.pdf"
        )
        if not path:
            return
        try:
            export_quiz_to_pdf(quiz['title'], qs, path)
            messagebox.showinfo('موفق', 'آزمون صادر شد.')
        except Exception as e:
            messagebox.showerror('خطا', f'خطا در صدور PDF: {e}')
