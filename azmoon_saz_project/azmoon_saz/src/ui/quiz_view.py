...
from utils.pdf_exporter import export_quiz_to_pdf
...
class QuizView(ctk.CTkFrame):
    ...
    def _build_footer(self):
        ...
        self.btn_export = ctk.CTkButton(left, text=fa('📄 خروجی PDF آزمون'), width=170, height=42,
                                        font=(FONT, 13, 'bold'), corner_radius=10,
    ...
    def _export_pdf(self):
        if not self.quiz or not self.questions:
            return
        path = filedialog.asksaveasfilename(
            defaultextension='.pdf',
            filetypes=[('PDF Files', '*.pdf')],
            initialfile=f"Quiz_{self.quiz.get('title', 'export')}.pdf"
        )
        if not path:
            return
        try:
            export_quiz_to_pdf(self.quiz.get('title', 'آزمون'), self.questions, path)
            messagebox.showinfo('موفق', 'آزمون با موفقیت به PDF صادر شد.')
