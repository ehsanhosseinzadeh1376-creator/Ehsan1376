...
class PdfView(ctk.CTkFrame):
    ...
    def _build_body(self):
        ...
        self.source_text = ctk.CTkTextbox(self.source_panel, font=(FONT, 13),
                                          fg_color='#0f1120', text_color='#dfe2ec',
                                          wrap='word', corner_radius=10)
        self.source_text.grid(row=1, column=0, sticky='nsew', padx=16, pady=(0, 16))
    ...
    def _render_extracted(self):
        ...
        if questions:
            for i, q in enumerate(questions, 1):
                self.source_text.insert('end', fa_textbox(f'{i}) {q["text"]}') + '\n')
                for opt in q.get('options', []):
                    self.source_text.insert('end', fa_textbox(f'   • {opt}') + '\n')
                self.source_text.insert('end', '\n')
        else:
            self.source_text.insert('1.0', fa_textbox('سوالی از قبل در PDF شناسایی نشد.'))
